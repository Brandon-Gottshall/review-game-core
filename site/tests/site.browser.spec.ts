...
test('home page exposes docs and feature entry points', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /docs-grade primitives/i })).toBeVisible()
  ...
  await expect(page.getByText(/Stats uses it for exam-track planning/i)).toBeVisible()
})
...
test('feature detail renders source snippet and consumer example', async ({ page }) => {
  ...
  await expect(page.getByText(/Goal types and evaluation helpers/i)).toBeVisible()
