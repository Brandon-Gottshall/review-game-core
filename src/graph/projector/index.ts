import type { GraphClient, GraphTransactionLike } from '../client.js'
import {
  GRAPH_SCHEMA_VERSION,
  type GraphProjectionEnvelope,
  graphProjectionEnvelopeSchema,
  graphProjectionStateSchema,
  graphProjectionStatusSchema,
  projectionRunMetadataSchema,
  type GraphProjectionState,
  type ProjectionRunMetadata,
...
const compactRecord = (value: Record<string, unknown>): Record<string, unknown> => Object.fromEntries(
  Object.entries(value).filter(([, entry]) => entry !== undefined)
)

const runBatch = async (
  tx: GraphTransactionLike,
  batch: WriteBatch
): Promise<void> => {
...
const buildStampedRows = <T extends Record<string, unknown>>(
  envelope: GraphProjectionEnvelope,
  rows: T[]
): Record<string, unknown>[] => rows.map((row) => compactRecord({
  ...row,
...
export class Neo4jGraphProjector implements GraphProjector, GraphProjectionRepository, GraphProjectionReconciler {
  constructor(private readonly client: GraphClient) {}

  async applyProjection(input: GraphProjectionEnvelope): Promise<ProjectionRunMetadata> {
  ...
  async clearGameGraph(gameId: string): Promise<void> {
    const session = this.client.session('WRITE')
    try {
      await session.executeWrite(async (tx) => {
...
export const createProjectionEnvelopeMetadata = (
  input: Omit<ProjectionRunMetadata, 'status' | 'schemaVersion'> & { schemaVersion?: number }
): ProjectionRunMetadata => projectionRunMetadataSchema.parse({
  ...input,
